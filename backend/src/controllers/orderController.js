  ...
  try {
    ...
    await session.withTransaction(async () => {
      ...
      for (const [productId, quantity] of quantities) {
        ...
        if (!product) {
          throw new OrderValidationError(
            400,
            "Un des produits de votre panier n'existe plus.",
          );
        }
      ...
      if (result.data.useLoyaltyPoints) {
        const loyaltyUser = await User.findById(request.user.id)
          .select("loyaltyPoints")
          .session(session);
        if (!loyaltyUser || loyaltyUser.loyaltyPoints < 1000) {
          throw new OrderValidationError(
            409,
            "Vous ne disposez pas de suffisamment de points fidélité.",
          );
        }
      ...
      if (result.data.promoCode) {
        ...
        if (!promo || (promo.expiresAt && promo.expiresAt < new Date())) {
          throw new OrderValidationError(400, "Code promo invalide ou expiré.");
        }
