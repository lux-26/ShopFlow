  ...
  const handleConfirmOrder = async () => {
    ...
    try {
      ...
      const resolvedCartItems = cartItems.map((item) => {
        const product =
          (typeof item.id === "string" && MONGO_ID_PATTERN.test(item.id)
            ? productsById.get(item.id)
            : null) ||
          (item.sku ? productsBySku.get(item.sku.toLowerCase()) : null);

        return product ? { ...item, id: product.id } : null;
      });
      const missingItems = resolvedCartItems.filter((item) => !item);

      if (missingItems.length > 0) {
        showToast(
          "Panier à actualiser",
          "Un article de votre panier n'existe plus dans le catalogue. Retirez-le puis ajoutez-le à nouveau.",
          "error",
        );
        return;
      }
